...
const Users = () => {
  ...
  const handleSubmit = async (values) => {
    ...
    let plan = JSON.parse(localStorage.getItem('org')).plan;
    if (appUsers.length == 1 && plan == 'free') {
      alert('You cannot add more users');
      return;
    }
    if (appUsers.length == 4 && plan == 'rise') {
      alert('You cannot add more users');
      return;
    }
    if (appUsers.length == 15 && plan == 'growth') {
      alert('You cannot add more users');
      return;
    }
    if (appUsers.length == 25 && plan == 'scale') {
      alert('You cannot add more users');
      return;
    }
